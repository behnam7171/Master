...
function generate_sections_page_template( $template ) {
	...
	if ( isset( $use_sections['use_sections'] ) && 'true' == $use_sections['use_sections'] ) {	
		...
		$new_template = dirname( __FILE__ ) . '/templates/generate-sections-page-template.php';
...
function generate_sections_styles() {
	...
	if ( isset( $use_sections['use_sections'] ) && 'true' == $use_sections['use_sections'] ) {	
		wp_enqueue_style( 'generate-sections-styles', plugin_dir_url( __FILE__ ) . 'css/style.css' );
		wp_enqueue_script( 'generate-sections-parallax', plugin_dir_url( __FILE__ ) . 'js/parallax.js', array('jquery'), GENERATE_VERSION, true );
	} 
}
endif;
...
if ( ! function_exists( 'generate_sections_save_function' ) ) :
	function generate_sections_save_function( &$item, $key ) {
		...
		if( isset( $item['top_padding'] ) ){
			$item['top_padding'] = intval( $item['top_padding'] );
  		}
			
		if( isset( $item['bottom_padding'] ) ){
			$item['bottom_padding'] = intval( $item['bottom_padding'] );
  		}
			
		if( isset( $item['background_image'] ) ){
			$item['background_image'] = intval( $item['background_image'] );
  		}
