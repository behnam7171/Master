...
function generate_save_de_meta($post_id) {  
	...
	if( isset( $_POST[ '_generate-disable-header' ] ) ) {
		update_post_meta( $post_id, '_generate-disable-header', 'true' );
	} else {
		update_post_meta( $post_id, '_generate-disable-header', '' );
	}
	
	if( isset( $_POST[ '_generate-disable-nav' ] ) ) {
		update_post_meta( $post_id, '_generate-disable-nav', 'true' );
	} else {
		update_post_meta( $post_id, '_generate-disable-nav', '' );
	}
	
	if( isset( $_POST[ '_generate-disable-secondary-nav' ] ) ) {
		update_post_meta( $post_id, '_generate-disable-secondary-nav', 'true' );
	} else {
		update_post_meta( $post_id, '_generate-disable-secondary-nav', '' );
	}
	
	if( isset( $_POST[ '_generate-disable-post-image' ] ) ) {
		update_post_meta( $post_id, '_generate-disable-post-image', 'true' );
	} else {
		update_post_meta( $post_id, '_generate-disable-post-image', '' );
	}
	...
	if( isset( $_POST[ '_generate-disable-footer' ] ) ) {
		update_post_meta( $post_id, '_generate-disable-footer', 'true' );
	} else {
		update_post_meta( $post_id, '_generate-disable-footer', '' );
	}
}  
endif;
