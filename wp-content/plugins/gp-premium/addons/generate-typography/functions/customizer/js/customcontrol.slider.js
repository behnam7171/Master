jQuery(window).load(function(){
	...
	var font_size_widget_content = jQuery('input[name="generate_settings[widget_content_font_size]"]').val();
	...
	var font_size_footer = jQuery('input[name="generate_settings[footer_font_size]"]').val();
	...
	setTimeout(function() {
		...
		// Widget content
		jQuery('input[name="generate_settings[widget_content_font_size]"]').next('div.slider').slider({
			value: font_size_widget_content,
			min: 6,
			max: 30,
			slide: function( event, ui ) {
				// Change input value and slider position
				jQuery('input[name="generate_settings[widget_content_font_size]"]').val( ui.value ).change();
				jQuery('#customize-control-generate_settings-widget_content_font_size .value').text( ui.value );
				// Change CSS in LivePreview			
				//$j("iframe").contents().find('body').css('font-size', ui.value+'px');
		...
		// footer
		jQuery('input[name="generate_settings[footer_font_size]"]').next('div.slider').slider({
			value: font_size_footer,
			min: 6,
			max: 30,
			slide: function( event, ui ) {
				// Change input value and slider position
				jQuery('input[name="generate_settings[footer_font_size]"]').val( ui.value ).change();
				jQuery('#customize-control-generate_settings-footer_font_size .value').text( ui.value );
				// Change CSS in LivePreview			
				//$j("iframe").contents().find('body').css('font-size', ui.value+'px');
