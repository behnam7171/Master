            ...
            ?>
                <label>
                    ...
                    <select <?php $this->link(); ?> class="fonts" data-value="<?php echo $this->value();?>">
						...
						<optgroup label="<?php _e( 'Google fonts', 'generate-typography' ) ?>" class="google_label">
							<?php
					...
					<p class="description"><?php _e('Font family','generate-typography'); ?></p>
                </label>
            <?php
        }
    }
...
if ( !class_exists('Generate_Customize_Slider_Control') ) :
	class Generate_Customize_Slider_Control extends WP_Customize_Control
	{
		...
		public function enqueue() {
			...
			wp_register_script( 'generate-customcontrol-slider-js', plugin_dir_url( __FILE__ ) . 'js/customcontrol.slider.js', array('jquery'), GENERATE_VERSION );
