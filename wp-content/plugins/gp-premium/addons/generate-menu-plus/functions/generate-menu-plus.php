...
function generate_menu_plus_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		...
		array(
			'type' => 'select',
			'label' => __( 'Sticky Menu', 'generate-menu-plus' ),
			'section' => 'menu_plus_section',
			'choices' => array(
	...
	$wp_customize->add_control(
		// ID
		'sticky_nav_effect_control',
		// Arguments array
		array(
			'type' => 'select',
			'label' => __( 'Sticky Menu Effect', 'generate-menu-plus' ),
			'section' => 'menu_plus_section',
			'choices' => array(
	...
	$wp_customize->add_control(
		// ID
		'sticky_nav_logo_control',
		// Arguments array
		array(
			'type' => 'select',
			'label' => __( 'Menu Logo Position', 'generate-menu-plus' ),
			'section' => 'menu_plus_section',
			'choices' => array(
	...
	$wp_customize->add_control(
		// ID
		'slideout_nav_control',
		// Arguments array
		array(
			'type' => 'select',
			'label' => __( 'Slideout Menu', 'generate-menu-plus' ),
			'section' => 'menu_plus_section',
			'choices' => array(
				'mobile' => __( 'Mobile only', 'generate-menu-plus' ),
				'desktop' => __( 'Desktop only', 'generate-menu-plus' ),
...
function generate_menu_plus_enqueue_js()
{
	...
	if ( 'false' !== $generate_menu_plus_settings['sticky_menu'] && 'none' !== $generate_menu_plus_settings['sticky_menu_effect'] ) :
		...
		wp_enqueue_script( 'generate-classie', plugin_dir_url( __FILE__ ) . 'js/classie.js', array(), GENERATE_MENU_PLUS_VERSION, true );
...
function generate_menu_plus_inline_css()
{
	...
	if ( 'contained-nav' == $generate_settings['nav_layout_setting'] && 'false' !== $generate_menu_plus_settings['sticky_menu'] ) :
		...
		$return .= '@media (min-width: ' . $generate_settings['container_width'] . 'px) { .nav-float-right .navigation-clone.main-navigation, .nav-float-right .main-navigation.is_stuck { float: none;left: 50%; width: 100%; max-width: ' . $generate_settings['container_width'] . 'px; margin-left: -' . $generate_settings['container_width'] / 2 . 'px; } }';
		$return .= '@media (min-width: 768px) and (max-width: ' . ($generate_settings['container_width'] - 1) . 'px) {.nav-float-right .navigation-clone.main-navigation { width: 100%; } }';
	endif;
...
if ( ! function_exists( 'generate_slideout_menu_overlay' ) ) :
add_action( 'wp_footer','generate_slideout_menu_overlay' );
function generate_slideout_menu_overlay()
{
	$generate_menu_plus_settings = wp_parse_args( 
		get_option( 'generate_menu_plus_settings', array() ), 
		generate_menu_plus_get_defaults() 
