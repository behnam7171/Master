...
(function ( $ ) 
{
	$.fn.GenerateStickyNavigation = function( options ) 
	{
		...
		var top = settings.cloned_element.position().top;
		
		// If our navigation is in a sidebar and we're on mobile, use the container
		if ( ( $( '.nav-right-sidebar' )[0] || $( '.nav-left-sidebar' )[0] ) && mobile.is( ':visible' ) ) {
			top = $( '#page.container' ).position().top;
		}
		
		// Get the top of our navigation when our navigation is floating
		if ( $( '.nav-float-right' )[0] || $( '.nav-float-left' )[0] ) {
			top = top + settings.cloned_element.parent().parent().position().top;
		}
		
		// Get the bottom of the navigation
		var bottom = top + settings.cloned_element.outerHeight() + 10;
