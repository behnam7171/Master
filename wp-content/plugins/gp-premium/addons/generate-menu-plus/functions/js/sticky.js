...
(function ( $ ) {
	$.fn.GenerateSimpleSticky = function( options ) {
		...
		var height = settings.menu.outerHeight(true);
		var el_float = settings.menu.css("float");
		spacer.css({
            'width': outer_width(settings.menu),
            'height': height,
            "vertical-align": settings.menu.css("vertical-align"),
            "float": el_float,
			'display': 'none'
          });
		
		if ( settings.spacer && ! spacer.length ) {
			$( this ).after( '<div class="sticky-spacer"></div>' );
			spacer_added = true;
		}
